...
{
    const TestBlockHashes bh{block.known_block_hashes};
    const auto result_or_error =
        state::transition(state, block, bh, tx, rev, vm, block_gas_left, blob_gas_left);
...
{
    const TestBlockHashes bh{block.known_block_hashes};
    const auto diff = state::system_call(state, block, bh, rev, vm);
    state.apply(diff);
}
}  // namespace evmone::test
